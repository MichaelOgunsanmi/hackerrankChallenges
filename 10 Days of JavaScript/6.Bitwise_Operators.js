...
const getMaxLessThanK = (n, k) => {
    const range = Array.from(Array(n), (value, index) => index + 1)
    const range2 = Array.from(Array(n - 1), (value, index) => index + 1)

    let max = 0

    for (let index = 0; index < range.length; index++) {
        const element = range[index];
        for (let index2 = 0; index2 < range2.length; index2++) {
            const element2 = range2[index2];

            if (element === element2) continue

            const check = element & element2

            check > max && check < k ? max = check : max = max
        }
    };
    return max
}
