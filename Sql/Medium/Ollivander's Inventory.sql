...
SELECT Wands.id, temp.age, Wands.coins_needed, Wands.power
FROM Wands
JOIN (
        SELECT MIN(age) as age, MIN(coins_needed) as m_n, power as power, code as code
        FROM Wands 
        JOIN Wands_Property USING (code)
        WHERE is_evil = 0
        GROUP BY power, code) temp
 ON temp.m_n = Wands.coins_needed and temp.power = Wands.power and temp.code = Wands.code
